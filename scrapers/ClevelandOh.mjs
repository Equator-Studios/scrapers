...
export default ({ database, DataScraper }) => {
  return DataScraper(database, "ClevelandOh", async () => {
    ...
    const request = `${baseUrl}/search?collection=Dataset&q=parcel&type=feature%20layer`;
    ...
    const requestsArcgisUrls = responsesArcgisUrls.map((request) =>
      request.json()
    );
    ...
    const requestsArcgisIds = responsesArcgisIds.map((request) =>
      request.json()
    );
