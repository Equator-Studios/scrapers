...
export default ({ database, DataScraper }) => {
	return DataScraper(database, 'StateOfTexas', async () => {
		...
		const catalouge = await fetch(`https://api.tnris.org/api/v1/collections_catalog/`);
		const catalougeData = await catalouge.json()
		const datasets = catalougeData.results.map(dataset => {
		...
		for (let i = 0 ; i < datasets.length; i++){ //forEach loops dont work with async
			...
			results.forEach(item => {
				console.log(item) //takes a long time so log each dataset to see progress
				data.push({
					url: item.resource,
					updated: 0,
					created,
