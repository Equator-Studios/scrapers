				...
				if (urlText && urlText === 'GIS') {
					if($(rows[i]).find(".mb5").find('a').attr("href") !== '/orthophotography') {
						...
						data.push({
							...
							updated: metadata.lastUpdateDate !== "Live" ? new Date(metadata.lastUpdateDate).getTime() : Date.now(),
							created: new Date(metadata.createdDate).getTime(),
							description: metadata.description + 'This data is provided by <a href="https://data.reddeer.ca/">https://data.reddeer.ca/</a>.',
							name: metadata.name
						});
					}
				}
